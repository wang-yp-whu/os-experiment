...
.081h:  Gate SelectorCode32, CustomHandler, 0, DA_386IGate
...
ClockHandler	equ	_ClockHandler - $$
	...
	iretd
_CustomHandler:
CustomHandler equ _CustomHandler - $$
    ...
    ; 橙色行
    mov cx, 80
    mov di, 160               ; 第1行（80字符*2字节=160字节偏移）
    mov ah, 0x0E              ; 前景色：橙色
    call PrintLine

    ; 黄色行
    mov cx, 80
    mov di, 320               ; 第2行（160字节偏移）
    mov ah, 0x0E              ; 前景色：黄色
    call PrintLine
...
PrintLine:
    push ax
    push di
    mov al, '*'               ; 要打印的字符，彩虹旗用'*'表示
.line_loop:
    mov [gs:di], ax           ; 将字符和颜色写入视频内存
    add di, 2                 ; 每个字符占2字节（字符+属性）
    loop .line_loop           ; cx == 0时退出
    pop di
    pop ax
    ret
